        ...
        <div className={cn(compact ? cn(hasRules && 'mb-2') : 'mb-3 sm:mb-4')}>
          ...
          <div className='space-y-1.5 sm:hidden'>
            {tiers.map((tier) => {
              ...
              const tierKey = `${normalizeTierLabel(tier.label) || 'default'}:${condSummary || JSON.stringify(tier.conditions)}`
              const isMatched =
        ...
        <div>
          ...
          <ul className='space-y-1.5'>
            {ruleGroups.map((group) => {
              const groupDescription = describeGroup(group, t)
              return (
                <li
                  key={`${groupDescription}:${group.multiplier}`}
